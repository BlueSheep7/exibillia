...
function load()
	...
	for index, this_story in pairs(love.filesystem.getDirectoryItems("stories")) do
		--create story ui buttons
	end
	
	view = L.ui.view{w = love.graphics.getWidth(), h = love.graphics.getHeight(),
		L.ui.button{title = "test", x = 100, y = 100},
	}
	view:open()
...
function draw()
	...
	if show_profile_settings then
		...
		if Failed_message then
			love.graphics.setColor(1, 1, 1)
			printF("Failed to load story.", getResX()/2 - Profile_settings_w/2, getResY()/2 - Profile_settings_h/2 + Profile_settings_h + 10)
		end
...
function mousepressed(mx_true, my_true, button)
	...
	if show_profile_settings then
		...
		if mx > getResX()/2 + Profile_settings_w/2 - 100 - 25 and mx < getResX()/2 + Profile_settings_w/2 - 100 - 25 + 100
		and my > getResY()/2 + Profile_settings_h/2 - 30 - 25 and my < getResY()/2 + Profile_settings_h/2 - 30 - 25 + 30 then
			loadStory()
			return true
		end
		...
		if button == 1 then
			...
			if L.story.replies then
				...
				if not typed_reply then
					...
					-- delete msg button --
					if Allow_Backspace and isWithin(mx, my, getResX() - Padding*2 - Img.ui.send:getWidth() - Img.ui.x:getWidth(), getResY() - Padding - Reply_box_h/2 - Img.ui.send:getHeight()/2, Img.ui.send:getWidth(), Img.ui.send:getHeight()) then
		...
		if L.story.replies then
			...
			if not typed_reply then
				...
				if key == L.settings.key_ui_enter then
					sendReply()
					return true
				elseif Allow_Backspace and key == L.settings.key_ui_backspace then
					backspaceReply()
					return true
				end
...
function loadStory()
	...
	if L.story.loadStory(selected_story, username, profile_pic) then
...
function selectReply(index)
	...
	if L.story.Force_Select_Choice and #L.story.replies == 1 then
		Allow_Backspace = false
	else
